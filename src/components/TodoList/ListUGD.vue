    ...
    <v-main class="list">
        ...
        <v-card>
            <v-card-title>
                ...
                ></v-text-field>
                <v-spacer></v-spacer>
                <v-btn color="purple" dark @click="dialogSelesai = true" style="margin: 1vh;">
                    TODO SELESAI
                </v-btn>
            ...
            <template v-slot:[`item.actions`]="{ item }">
                ...
                <v-btn small @click="deleteItem(item); indexItem = todos.indexOf(item);">
                    delete
                </v-btn>
            </template>
            </v-data-table>
        ...
        <v-dialog v-model="dialogSelesai" persistent max-width="600px">
            <v-data-table :headers="headers" :items="todosSelesai" :search="search">
                 <template v-slot:[`item.priority`]="{ item }">
                    <v-chip v-if="item.priority == 'Penting'"
                        class="ma-2"
                        color="red"
    ...
    methods: {
        save() {
            ...
            for(i = 0; i < this.todos.length; i++) {
                if(this.todos[i].task==this.formTodo.task) {
                    console.log("i: " + i);
        ...
        deleteItem(item){
            ...
            // this.dialogDelete = false;
            this.todosSelesai.push(this.todos.indexOf(this.item));
            confirm('Yakin ingin menghapus?') && this.todos.splice(this.todos.indexOf(item), 1);
        },
    },
};
</script>